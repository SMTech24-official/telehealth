...
import { Tag } from "./Tag"
import JoinConsultation from "../JoinConsultaton"



export default function ConsultationBook({ title, fee, nextConsultation, services, timeSlots, history, booking, }: { title: string, fee: number, nextConsultation: string, services: string[], timeSlots?: string[], history: boolean, booking?: boolean }) {
    return (
                ...
                <div className="flex lg:flex-row flex-col  lg:items-center justify-between gap-6">
                    <div className="relative flex flex-col md:flex-row lg:items-center gap-6">
                        <Image
                            ...
                            alt={`image of ${title}`}
                            className="object-contain h-36 lg:h-56"
                        />
...

                    {
                        history && <div className="space-y-4">
                            <h2 className="text-xl font-semibold text-gray-900">Today Available Time</h2>
                            <div className="flex flex-wrap lg:grid grid-cols-3 items-center gap-3">
                                {timeSlots?.map((time, index) => (
                                    <Tag key={index} name={time} />
                                ))}
                            </div>
                        </div>
                    }
                    {
                        booking && <div className="space-y-4">
                            <JoinConsultation />
                        </div>
                    }

                    ...
                    <div className="xl:max-w-2xl xl:mx-auto xl:space-y-8 lg:flex-col md:flex hidden ">
                        {blogPosts.map((post) => (
                            <Link
                                key={post.slug}
                                href={`/blog/${post.slug}`}
                                className={`flex group items-center justify-center  gap-4 p-4 rounded-lg hover:bg-secondary transition-colors ${currentImage === (Number.parseInt(post.id) - 1) ? "bg-secondary/50" : ""}`}
                            >
